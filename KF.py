...
for iteration, data in enumerate(test_loader):
    curr_loss = 0
    model = KalmanFilter(world).to(device).double()
    ...
    coeff = 1
    d_loc = coeff * torch.randn(1, 3).to(device).double()
    d_loc[:, -1] = 0
...
for current_length in np.arange(10, 60, 10):
    ...
    print('   ', current_length, np.array(int_loss[current_length]).mean(), int_loss[current_length].std(), fin_loss[current_length].mean(), fin_loss[current_length].std())